...
- **最終作業タスク**: 実装フェーズ（2.x/3.x/4.1/4.2/4.4）完了 + 統合検証 pass（test 16/16・lint・build）。実装 16/20
- **中断理由**: 実装完了。次は Codex 差分レビュー（手順4）→ 5.1 適用ゲート
- **次のアクション**: ①Codex 差分レビュー（背景・受入条件・canonical 制約を差分と一緒に渡す）→ 採否判断 → 最大3サイクル。②5.1 適用ゲート: **007 を prod apply → `select toilet_indexable_count();` が 1,434 に一致疎通 → build（chunk=2 確認、0 なら deploy 中止）→ deploy → /robots.txt・/sitemap/1.xml smoke**。③4.3 SQL 同値（007 apply 後に §5.2 を SQL Editor 再現、U+3000 実値確認）。④PR（Closes #1）。**007 apply / deploy は覇王承認のもとで（独断実行しない）**
