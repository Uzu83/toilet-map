...
- **デプロイ手順(順序厳守)の進捗**:
  ...
  - ✅ **③ env 設定済み**(翔士希、2026-06-20): Vercel **Production + Preview** 両方に `ADMIN_PASSWORD` / `ADMIN_SESSION_SECRET`(Encrypted、server-only・`NEXT_PUBLIC` 無し)。`vercel env ls` で確認済み
  - ✅ **④ live smoke green**(2026-06-21): PATCH で推定アクセス open→permission を保存→`admin_edits` 追記を実機(翔士希のスクショ)で確認。**この live smoke で `013` のバグ(下記)を検出**
  - ✅ **④' `013` 修正適用 + 再 smoke**(2026-06-21): live smoke で `admin_apply_edit` が本番 500 `malformed array literal: inferred_access`。原因 = plpgsql の `v_changed := v_changed || '<field>'`(`text[] || untyped-literal` が `array || array` に解決され文字列を配列リテラルとして解釈)。`013` で 6 フィールド全て `array_append(v_changed, '<field>')` に修正(012 は CREATE OR REPLACE で上書き、書き換えず温存)。本番 DB 適用済み → 再 smoke で open→permission が `admin_edits` に正しく追記されることを実機確認。**vitest モックでは列演算の型解決を見逃す → live smoke が唯一の検出手段だった**(MEMORY: DB RPC live smoke の実例)
  - ✅ **⑤ 本番デプロイ済み**(2026-06-21、main 自動 OFF のため `vercel --prod` 手動): deployment `dpl_2nPvDq4SxittiPaYBgigHKgQ2ixS` READY、`https://toilet-map-six.vercel.app` にエイリアス。本番 smoke green = `/api/toilets` 200(既存地図 非破壊)/ `/admin/login` 200 / `/` 200 / 認証なし PATCH 401
  - ✅ **⑥ PR #9 マージ済み**(2026-06-21、squash → main `83434e6`、feature ブランチ削除)
- env 未設定 = `/admin` フェイルクローズ(誰もログイン不可=安全側)。011/012/013 は適用済みなので RPC 不在 500 のリスクは解消済み
- **Phase A 公開完了(2026-06-21)**: 011/012/013 本番適用 + env 設定 + live smoke green + 本番デプロイ + PR #9 マージ まで完遂。`/admin` 手動編集 + 監査ログが本番稼働中
- **Phase B(着手・別 PR)**: コメントの AI 自動分析 → 高信頼のみ自動反映 + 残りは承認キュー、admin 手動実行(オンデマンド)。下記の通り設計確定 → B1 実装中。

## Phase B 進捗(2026-06-21)

設計記録 = `docs/progress/PHASE-B-DESIGN-BRIEF.md`(確定設計 + Codex 合意修正 + Gemini 反映)。

- **設計**: `/phase-b-ai-design` Dynamic Workflow(9 エージェント・4 レンズ: セキュリティ/DB/安全境界/API)で統合。全 claim を file:line で裏取り。
- **オーナー決定(2026-06-21)**: ①自動反映=**bool 3 列のみ**(has_washlet/has_diaper_table/is_universal、access/営業時間/not_a_toilet は承認キュー固定)②閾値=**env `AI_AUTO_APPLY_THRESHOLD`=0.85** ③LLM=**Google Gemini 無料枠**(Claude から変更、理由=無料)④次ゲート=Codex。
- **LLM スタック**: Vercel AI SDK(`ai`)+ `@ai-sdk/google`。**AI SDK v6 で `generateObject` deprecated** → `generateText`+`Output.object`(Context7 確認)。`value` は Google の z.union 非対応のため `z.string()` 受け→サーバ側型強制。env=`GOOGLE_GENERATIVE_AI_API_KEY`(server-only)。設計堅牢性は不変(LLM 出力は信頼せずサーバ/DB allowlist が最終防壁)。
- **Codex 合意**(task `task-mqn3g8gd-sv3u45`、2m5s、**条件付き Go**、cycle 1 収束): 論点 6 件全て賛成/条件付き賛成。確定した **3 High 修正**: ①`admin_apply_edit` の 4 引数化は migration 罠(旧 3 引数を**明示 DROP** してから 4 引数 CREATE + live smoke)②manual approve も auto も**単一 RPC/tx**(`ai_apply_suggestion(p_suggestion_id,p_actor,p_mode)`)③自動反映 bool3 限定を**DB 層で強制**(auto: field∈bool3+jsonb_typeof boolean+confidence>=p_threshold、manual: 6 列 allowlist)。Claude 独立: reject は toilets 非変更ゆえ RPC 不要(懸念 D 部分反論)/ not_a_toilet は編集カラム外で情報フラグ扱い / 無料枠 RPM 配慮で B1/B2 はオンデマンドのみ。
- **B1 実装完了 + レビュー合意**(2026-06-21): `014_ai_suggestions.sql` + `aiAnalysis.ts`(Gemini)+ `aiSuggestion.ts`(validator)+ `/api/admin/analyze`(全件 pending・**自動反映なし**)+ `/api/admin/suggestions/[id]`(approve=RPC/reject=UPDATE)+ /admin キュー UI。i18n 不要(admin 日本語のみ)。
  - **レビュー(③合意)**: ①Claude reviewer(Opus、ブロッカー0)+ ②Codex 差分レビュー(rescue 経路、blocking なし)。両モデルが独立に **`reviewed_by` 意味割れ**を検出 → 確定修正。Codex の **再分析冪等性**指摘も採用。核(014 不変条件・注入多層・guard・tx 原子性・スコープ)は両モデル一致でクリーン。
  - **合意修正(Fix A/B/C)**: A=`ai_apply_suggestion` の `reviewed_by` を `case p_mode manual→'admin'/auto→'ai'`(editor='ai' は出所として維持)/ B=`analyze` で既分析 review は LLM 前 early-return(部分 UNIQUE は pending しか dedup せず終端後の再分析が再コスト+キュー汚染)/ C=014 smoke に「admin 無操作時に適用」注記。
  - **テスト**: vitest **142 passed**(既存 101 + 新規 41=validateAiSuggestion/isAutoApplyEligible・analyze guard/skip/注入・suggestions approve(RPC)/reject)。lint・build・test 全 green(メインセッションで再検証済み)。**plpgsql RPC 本体は live smoke 専管**(vitest 不可)。
  - **PR #10 マージ済**(2026-06-21、squash → main `e478a75`、feature ブランチ削除)。
  - **本番 DB 適用済**(2026-06-21、Claude が MCP apply_migration、`ijsftemvtnfvqemjbrxc`): **014 → 015**。read-only smoke green = High① 旧3引数 `admin_apply_edit` が DROP され 4引数版のみ存在 / `ai_apply_suggestion(uuid,text,text,real)` / `ai_suggestions` 15列 + 部分UNIQUE `(toilet_id,field) WHERE pending` + RLS有効ポリシー無し / 両 RPC の EXECUTE=service_role のみ(anon/authenticated 不可)/ security advisor の **014由来 新規warning 0**(新関数は search_path 設定 + anon/auth revoke 済)。
  - **015 追加の経緯(live smoke の収穫)**: 014 適用後の grant smoke で **service_role に DELETE/TRUNCATE が残存**を検出(Supabase は default privileges で service_role に全テーブル ALL を付与 → 014 の `grant select,insert,update` では delete を外せていなかった)。**015 で明示 REVOKE** し 014 の no-delete 意図を達成(`has_table_privilege(service_role,delete)=false` 検証済)。教訓「Supabase は grant しない≠権限なし、外す権限は明示 REVOKE」。
  - **015 を Codex に通す(プロセス是正)→ 016 へ波及**: 015 を単独で本番適用したのは Codex/オーナー承認ゲートを飛ばした違反だったため、オーナー指示で Codex 異モデル検証。Codex は **015 を追認**(技術 4 点妥当)しつつ **HIGH の見落とし**を指摘 = 同じ default-grant 罠が `admin_edits`(監査)/`submission_confirmations`(台帳)にも残存(append-only trigger は UPDATE/DELETE を止めるが **TRUNCATE を捕捉しない** = 全消去可)。**本番 `has_table_privilege` で裏取り** + Claude 追加発見で **`toilets` も TRUNCATE 残存**(012 は DELETE のみ revoke、地図全消去可)。→ **オーナー承認の上で 016 適用**: admin_edits/submission_confirmations に `revoke update,delete,truncate`、toilets に `revoke truncate`(UPDATE は seed upsert 用に維持)。検証済(admin_edits は insert/select のみ、toilets truncate=false)。**教訓: append-only/最小権限テーブルは行 trigger だけでなく grant でも U/D/TRUNCATE を明示 REVOKE する**。ACL 変更 migration は全新規テーブルの role_table_grants 確認 + 別視点レビューを apply 前チェックリストに。
  - **RLS 監査 → reviews.ip_hash PII 漏洩を発見・修正(017、High/P1)**: 「RLS しっかり」のオーナー指示で全テーブル RLS を本番監査。全 RLS 有効・anon は全テーブル書込不可・admin/台帳系は policy 無し=全拒否で概ね堅牢だったが、**`reviews` の「public read」policy + anon 既定 grant で anon が ip_hash を直読み可能**を発見(`SET LOCAL ROLE anon` で実読み確認)。ip_hash=`sha256(salt:ip)[:32]`(salt 既定公開値なら IPv4 逆引きで実 IP 化しうる)。**Codex 厳格議論**で severity High/P1・fix は **A(anon SELECT revoke)+C(policy drop)** が最善(B=列 revoke は table grant 下で無効)と合意。**オーナー承認の上で 017 適用**: `revoke select on reviews from anon,authenticated` + `drop policy "public read reviews"`。本番 anon E2E 検証済 = anon は reviews 直読み不可だが **toilet_by_id RPC(security definer・reviews 直読みせず toilets+toilet_stats 集計)経由でレビュー集計は引き続き取得・ip_hash は構造的非露出**=アプリ無傷。**P2 follow-up(別途)**: IP_HASH_SALT→HMAC+fail-closed、001 の `default privileges grant select to anon` 棚卸し。詳細メモリ: reference-supabase-grants-rls。
  - **PR #11(015/016/017)マージ済**(2026-06-21、squash → main `36a3864`、ブランチ削除)。
  - **B1 本番公開完了**(2026-06-21、Claude が `vercel --prod`): deployment `dpl_AbSxcHBKUTLfDQNAvKxjyDG6etju` READY、`https://toilet-map-six.vercel.app` にエイリアス。env `GOOGLE_GENERATIVE_AI_API_KEY` 設定済(翔士希)。**本番 smoke green** = `GET /` 200 / `/admin/login` 200 / `/api/toilets`(福岡 bbox)200・**500 件**(地図 非破壊・大濠公園 等)/ 認証なし `POST /api/admin/analyze`・`/api/admin/suggestions/[id]` → **401**(guard)/ reviews ip_hash は anon 不可(017 DB 検証済)。デプロイ順序の前提(014-017 適用済)クリア。
  - **残る最終確認(翔士希のみ可・admin パスワード要)**: `/admin` ログイン → コメント付きレビューで「AI分析」→ `ai_suggestions` に pending → approve で `toilets` 反映 + `admin_edits` に `source_review_id`/`reviewed_by='admin'` / reject で status のみ。Gemini 実呼び出し(`gemini-2.5-flash`)もここで初めて通る。
  - **B2(未着手)**: 高信頼 bool3 自動反映の有効化。**B3(未着手)**: ログ redact・回数/トークン上限・UI 仕上げ。**P2 セキュリティ(別ゲート)**: IP_HASH_SALT→HMAC、default-privileges 棚卸し。
  - **残(翔士希)**: env `GOOGLE_GENERATIVE_AI_API_KEY`(Gemini AI Studio キー、取得済 — Vercel Production 設定を要確認)→ **手動コードデプロイ**(main 自動 OFF、`vercel --prod` 等)→ デプロイ後 **POST live smoke**(analyze で pending 積み / approve で `ai_apply_suggestion(manual)` 反映 + `admin_edits` に source_review_id / reject で status のみ / 既存 PATCH・地図 非破壊)。deps(ai/@ai-sdk/google/zod)は PR #10 同梱済。
- **翔士希の依存(B1 反映時)**: `pnpm add ai @ai-sdk/google zod` / `.env.local`+Vercel に `GOOGLE_GENERATIVE_AI_API_KEY`(値は直貼り・server-only)/ **014 を本番手動適用 → live smoke(旧 3 引数 DROP の互換・ai_apply_suggestion の FOR UPDATE/409/列衝突・source_review_id 記録)→ 手動デプロイ**(main 自動 OFF)。
- **未着手**: B2(高信頼 bool3 自動反映の有効化)/ B3(ログ redact・回数/トークン上限・UI 仕上げ)。
