...
- **マージ前に人間が必須**(順序厳守): ①Supabase に `011`→`012` 手動適用 → ②smoke → ③Vercel + `.env.local` に env `ADMIN_PASSWORD`/`ADMIN_SESSION_SECRET`(server-only・`NEXT_PUBLIC` 禁止)→ ④**live smoke**(login→cookie / PATCH→`admin_edits` 追記 / no-op 200 changed:[] / DELETE 取消の 409 / 既存 `/api/toilets` 非破壊)→ ⑤手動デプロイ(main 自動 OFF)→ ⑥PR マージ
- 011/012 未適用デプロイ = `PATCH/DELETE /api/admin/toilets/[id]` が RPC 不在で 500。env 未設定 = `/admin` フェイルクローズ(誰もログイン不可=安全側)
- **Phase B(未着手・別 PR)**: コメントの AI 自動分析 → 高信頼のみ自動反映 + 残りは承認キュー、admin 手動実行(オンデマンド)
