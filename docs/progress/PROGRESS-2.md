...
| 4.2 | /codex:review（実装差分） | ✅ | 3サイクル実施・全7指摘採用/修正。下記ログ参照 |
| 4.3 | reviewer サブエージェント監査 | ✅ | P1=0/P2=0、push OK。P3(未使用キー pendingBadge)を配線で解消 |
| 4.4 | **CLAUDE.md 更新**（lint/build/test 後） | ✅ | Phase 表に Phase2 実装済マーク + モデレーション運用セクション新設(多層防御/承認/却下/insert-only/残存リスク/SEO)。migration を 9 ファイルに更新 |
| 4.5 | **デプロイ順序の遵守** | ✅(文書化) | CLAUDE.md に 008→009→smoke→コードデプロイ + submit_toilet service_role 限定の注意を明記。**実デプロイ(migration 手動適用)は人間/デプロイ時に実施** |
| 4.6 | PR 作成 | ✅ | [PR #4](https://github.com/Uzu83/toilet-map/pull/4) — `Closes #2`、自動生成マーク、lint/build/test PASS、Codex 3サイクル要約、デプロイ手順を記載 |
| 4.7 | マージ → デプロイ → 進捗表更新 | ✅ | PR #4 マージ済 → Supabase に 008/009 + **010(本番バグ修正)** 適用 → 本番スモーク合格。Notion 反映は別途 notion-sync |
...
- **進捗サマリ**: 完了 = 準備3 + 実装15 + テスト3(+3.3 はSQL検証) + 4.1 + 4.2。残: 4.3 reviewer → 4.4 CLAUDE.md → 4.5 デプロイ → 4.6 PR → 4.7 Notion
- **ブロッカー**: なし

### 2026-06-14 (Codex 差分レビュー 4.2, 3サイクル)
全 7 指摘すべて採用・修正(誤指摘なし)。各サイクルで重大度が低下(P1→P2→P2)=健全な収束。修正後 lint/build clean・test 39/39。
- **サイクル1**: [P1] `submit_toilet` が PUBLIC 既定 EXECUTE で anon 直叩き可(API 迂回 + p_ip_hash 偽装) → REVOKE 追加 / [P2] DB スロットル時に IP 枠を先消費し「300秒後再試行」が1時間阻まれる → peekLimit+recordHit に分離、成功時のみ消費
- **サイクル2**: [2-a] `nearby_toilet` が非表示(not_a_toilet>=5)の偽陽性も dup 判定し正当な再登録を阻む → `<5` 可視性述語で除外(E7 挙動を反転) / [2-b] advisory lock の丸め境界で 30m 以内が別ロック→pending 重複 → point±pad 全セルロックに拡張
- **サイクル3 (上限)**: [3-a] XFF 先頭詐称で distinct-ip confirm 水増し→偽承認 → `x-real-ip` 優先(緩和)。完全な Sybil 耐性は Phase3 Auth へ(受容済みリスク #3) / [3-b] 高緯度で経度 pad<30m → cos(lat) 補正。日本(〜45.5°N)では実害なしだが一般化
- **エスカレーション判断**: 3サイクル上限到達。ただし各サイクルは別個の新規指摘でブロッカーの未収束ではない。全件修正済み。残存は文書化済み Phase3 項目のみ → /codex:rescue・人間エスカレーションは不要と判断

### 2026-06-14 (マージ → 本番デプロイ → 本番バグ修正)
- PR #4 マージ → Vercel 自動デプロイ READY。Supabase MCP で **008 → 009 を本番適用**(toilets 実件数 80,450 で依存 OK 確認)。anon は submit_toilet 実行不可(REVOKE 有効)を本番確認。
- **本番スモークで実バグ検出**: POST `/api/submissions` が 500 `column reference "submission_id" is ambiguous`。submit_toilet の OUT 列名がテーブル列名と衝突。vitest は RPC モックで DB ロジック未実行=すり抜け。`#variable_conflict use_column` で修正し **010** として本番反映(再デプロイ不要)→ 再スモーク合格(POST 200/連投 429/不正 400、GET 200、/api/toilets 87件 回帰なし)。
- **教訓**: DB RPC を持つ機能は vitest モックだけでなく本番/staging スモークで DB ロジックを必ず検証する(モックは plpgsql の変数/列衝突を捕捉できない)。
- **再発防止**: `vercel.json` で main の自動デプロイを OFF(`git.deploymentEnabled.main=false`)。本番は「migration 手動適用 → smoke → 手動デプロイ」に。preview は残す。
- スモークで作成したテスト申請(1件)は status='rejected' に soft-reject 済(pending 一覧から除外)。

---

## 作業再開ガイド

- **最終作業タスク**: 実装 + テスト + Codex 3サイクル + reviewer 監査 + ドキュメント完了。**PR #4 提出済**。
- **中断理由**: PR レビュー/マージ待ち(人間)。AI 側の実装フローは完了。
- **次のアクション(人間/マージ後)**:
  1. PR #4 をレビュー・マージ
  2. **デプロイ前に Supabase へ 008 → 009 を手動適用** → smoke test(申請 POST 200/201, `/api/submissions?bbox=` 200, `/api/toilets` 健全) → コードデプロイ
  3. notion-sync で Loo map ページに Phase 2 実装済を反映(4.7)
- **残存(Phase 3 委譲)**: 完全な Sybil 耐性(XFF 信頼境界・Auth)、本格モデレーション管理 UI、3.3 系の DB smoke 自動化

### 再開コマンド
```bash
git checkout feature/2 && git pull
```
