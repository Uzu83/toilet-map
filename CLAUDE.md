...
- **main は自動デプロイ ON(`vercel.json` の `git.deploymentEnabled.main=true`)**（2026-09-06 変更。以前は OFF）: feature ブランチの preview は従来どおり。**migration 依存の変更は merge より先に** ①Supabase へ新規 migration を手動適用 → ②smoke → ③main へ merge（merge が本番デプロイ）。未適用のまま merge すると RPC 不在で 500。手動デプロイ（Vercel dashboard の "Deploy"/"Redeploy"、`vercel --prod`、MCP `deploy_to_vercel`）は緊急時・設定切替時の手段として残す。
...
| `main` ブランチの自動デプロイ ON（2026-09-06） | merge が本番デプロイ。migration 未適用のまま merge すると RPC 不在 500。適用+smoke を merge より先にする | `## Supabase 運用` の「main は自動デプロイ ON」 |
